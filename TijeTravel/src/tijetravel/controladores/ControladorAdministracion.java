...
public class ControladorAdministracion {
    ...
    public ControladorAdministracion(Agencia agencia) { this.agencia = agencia; this.reservas = new ControladorReservas(agencia); }

    public boolean agregarSucursal(Usuario u, Sucursal s) { return puede(u, Permiso.ADMINISTRAR_SUCURSALES) && agencia.agregarSucursal(s); }
    public boolean modificarSucursal(Usuario u, int c, String d, String t) {
        if (!puede(u, Permiso.ADMINISTRAR_SUCURSALES)) return false;
        Sucursal s=agencia.buscarSucursalPorCodigo(c); return s!=null && s.actualizarDatos(d,t);
    }
    public boolean eliminarSucursal(Usuario u,int c){return puede(u,Permiso.ADMINISTRAR_SUCURSALES)&&agencia.eliminarSucursal(c);}

    public boolean agregarHotel(Usuario u, Hotel h){return puede(u,Permiso.ADMINISTRAR_HOTELES)&&valido(h)&&agencia.agregarHotel(h);}
    public boolean modificarHotel(Usuario u,int c,String n,String d,String ciudad,String tel,int plazas){
        if(!puede(u,Permiso.ADMINISTRAR_HOTELES)||plazas<reservas.ocupacionMaximaHotel(c))return false; Hotel h=agencia.buscarHotelPorCodigo(c);if(h==null)return false;
    ...
    public boolean modificarVuelo(Usuario u,int n,LocalDateTime f,String o,String d,int total,int turista,int primera){
        if(!puede(u,Permiso.ADMINISTRAR_VUELOS)||total<=0||turista<reservas.contarReservasVuelo(n,ClaseVuelo.TURISTA)
                ||primera<reservas.contarReservasVuelo(n,ClaseVuelo.PRIMERA)||turista+primera>total)return false;
