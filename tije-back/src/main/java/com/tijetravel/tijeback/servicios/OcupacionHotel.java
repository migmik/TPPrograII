...
/** Intervalos [llegada, partida): salir y entrar el mismo dia no ocupa dos plazas. */
final class OcupacionHotel {
    ...
    static int maxima(List<Reserva> reservas, LocalDate desde, LocalDate hasta, Integer ignorada) {
        ...
        for (Reserva reserva : reservas) {
            if (ignorada != null && ignorada.equals(reserva.getCodigo())) continue;
            LocalDate inicio = reserva.getFechaLlegada();
            LocalDate fin = reserva.getFechaPartida();
            if (desde != null && inicio.isBefore(desde)) inicio = desde;
            if (hasta != null && fin.isAfter(hasta)) fin = hasta;
            if (!inicio.isBefore(fin)) continue;
